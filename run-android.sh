...
nix-shell -p jre8 --run "$app/android/androidsdk/libexec/tools/android list devices"

# include /usr/sbin to path to provide system_profiler on osx
nix-shell -p jre8 which file --run "\
     ANDROID_SDK_ROOT=\$(pwd)/$app/android/androidsdk/libexec $app/android/androidsdk/libexec/tools/android create avd -f -n $app -b default/x86_64 -d \"$androidDevice\" -t \$($app/android/androidsdk/libexec/tools/android list targets -c | tail -n 1)\
